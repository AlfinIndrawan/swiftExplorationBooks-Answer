...
protocol PixelOperationQueue {
    ...
    func enqueue(onMainThread: Bool, _ operation: @escaping () -> Void)
...
class SimplePixelOperationQueue: PixelOperationQueue {
    ...
    func enqueue(onMainThread: Bool = false, _ operation: @escaping () -> Void) {
        queue.async {
            if onMainThread {
...
public class ThreadedPixelOperationQueue: PixelOperationQueue {
    ...
    public func enqueue(_ operation: @escaping () -> Void) {
        safe = false
        operations.append {
            operation()
            self.next()
