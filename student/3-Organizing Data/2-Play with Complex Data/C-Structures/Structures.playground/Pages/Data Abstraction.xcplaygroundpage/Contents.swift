...
let durations = [10,12,13,14]


func songInformation(_ i: [Int]) -> String {
    let song = Song(title: songTitles[i], artist: artists[i], duration: durations[i])
    return "\(song.title) artist is \(song.artist) with duration \(song.duration)"
}



for i in 0 ... songTitles.count - 1 {
    print(songInformation([i]))
}
